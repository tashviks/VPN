...
public abstract class TcpTunnel implements KeyHandler {
    ...
    public void connect(InetSocketAddress destAddress) throws Exception {
        ...
        if (VpnServiceHelper.protect(mInnerChannel.socket())) {
            ...
            //DebugLog.i("DestAddress %s", mDestAddress);
        } else {
    ...
    public void onReadable(SelectionKey key) {
        try {
            ByteBuffer buffer = ByteBuffer.allocate(ProxyConfig.MUTE);
            buffer.clear();
    ...
    public void getWriteDataFromBrother(ByteBuffer buffer) {
        //如果没有数据尝试直接写
